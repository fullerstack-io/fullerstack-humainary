 ...
 *
 * <p>
 * The @Fluent annotation indicates that a method returns `this` (the same
 * instance) to enable fluent method chaining. This is a critical contract that
 * ensures:
 *
 * <ul>
 ...
 * <p>
 *
 * @Fluent annotated methods are found in:
 *
 *         <ul>
 *         <li>{@link Flow} - diff(), guard(), limit(), peek(), reduce(),
 *         replace(), sample(), sift(), skip()
 *         <li>{@link Sift} - above(), below(), high(), low(), max(), min(),
 *         range()
 *         </ul>
 */
...
class FluentAnnotationContractTest {
  ...
  class FlowFluentMethods {
    ...
    void diff_returnsSameInstance () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
    ...
    void guardWithInitial_returnsSameInstance () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          ...
          assertSame ( original, result, "guard(initial, predicate) must return same Flow instance" );
        } );
    }
    ...
    void reduce_returnsSameInstance () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          ...
          assertSame ( original, result, "reduce(initial, operator) must return same Flow instance" );
        } );
    }
    ...
    void sift_returnsSameInstance () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          ...
          assertSame ( original, result, "sift(comparator, configurer) must return same Flow instance" );
        } );
    }
    ...
    void fluentChaining_worksCorrectly () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          Flow < Integer > original = flow;
          Flow < Integer > result = flow.diff ().limit ( 100 ).sample ( 10 ).skip ( 5 ).peek ( v -> {
          } ).replace ( v -> v * 2 );
      ...
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          flow.sift ( Comparator.naturalOrder (), sift -> {
            ...
            assertSame ( original, result, "range(lower, upper) must return same Sift instance" );
          } );
        } );
    }
    ...
    void siftChaining_worksCorrectly () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          flow.sift ( Comparator.naturalOrder (), sift -> {
            Sift < Integer > original = sift;
            Sift < Integer > result = sift.min ( 0 ).max ( 100 ).above ( 10 ).below ( 90 ).high ().low ().range ( 20, 80 );
            assertSame ( original, result, "Chained sift methods must return same instance" );
          } );
        } );
    }
  }
  ...
  class CombinedFluentTests {
    ...
    void complexChain_maintainsIdentity () {
      Conduit < Pipe < Integer >, Integer > conduit = circuit.conduit ( cortex ().name ( "conduit" ), Composer.pipe (),
        flow -> {
          Flow < Integer > original = flow;
          Flow < Integer > result = flow.diff ().limit ( 1000 ).sift ( Comparator.naturalOrder (), sift -> {
            Sift < Integer > siftOriginal = sift;
            Sift < Integer > siftResult = sift.min ( 0 ).max ( 100 ).high ();
            assertSame ( siftOriginal, siftResult, "Sift chain must return same instance" );
          } ).sample ( 10 ).skip ( 5 );
