 ...
 *
 * <p>
 * The @New annotation indicates that a method MUST return a new instance every
 * time it is called. This is a critical contract that ensures:
 *
 * <ul>
 ...
 *
 * <p>
 * These tests verify our implementation honors this contract for all @New
 * annotated methods.
 */
...
class NewAnnotationContractTest {
  ...
  class CortexNewMethods {
    ...
    void circuit_returnsNewInstance () {
      ...
      try {
        ...
        assertNotSame ( circuit1.subject (), circuit2.subject (), "Circuits should have different subjects" );
      } finally {
        circuit1.close ();
        circuit2.close ();
      }
    ...
    void scope_withName_returnsNewInstance () {
      ...
      try ( Scope scope1 = cortex ().scope ( name ); Scope scope2 = cortex ().scope ( name ) ) {
  ...
  class CircuitPipeMethods {
    ...
    void pipe_withNameTargetAndConfigurer_returnsNewInstance () {
      ...
      assertNotSame ( pipe1, pipe2, "pipe(name, target, configurer) must return new instance each call" );
    }
    ...
    void pipe_withNameReceptorAndConfigurer_returnsNewInstance () {
      ...
      assertNotSame ( pipe1, pipe2, "pipe(name, receptor, configurer) must return new instance each call" );
    }
  }
  ...
  class CircuitConduitMethods {
    ...
    void conduit_withoutNameWithConfigurer_returnsNewInstance () {
      ...
      assertNotSame ( conduit1, conduit2, "conduit(composer, configurer) must return new instance each call" );
    }
    ...
    void conduit_withConfigurer_returnsNewInstance () {
      ...
      Conduit < Pipe < String >, String > conduit1 = circuit.conduit ( conduitName, Composer.pipe (), flow -> {
      } );
      Conduit < Pipe < String >, String > conduit2 = circuit.conduit ( conduitName, Composer.pipe (), flow -> {
      } );

      assertNotSame ( conduit1, conduit2, "conduit(name, composer, configurer) must return new instance each call" );
    }
  }
  ...
  class CircuitSubscriberMethods {
    ...
    void subscriber_returnsNewInstance () {
      ...
      BiConsumer < Subject < Channel < Integer > >, Registrar < Integer > > callback = ( subject, registrar ) -> registrar
        .register ( value -> {
        } );
  ...
  class CircuitSubscribeMethods {
    ...
    void subscribe_returnsNewSubscription () {
      Subscriber < State > subscriber = circuit.subscriber ( cortex ().name ( "stateLogger" ),
  ...
  class StateNewMethods {
    ...
    void state_withBoolean_returnsNewInstance () {
      ...
      assertNotSame ( state1, state2, "state.state(name, boolean) must return new instance each call" );
    }
    ...
    void state_compact_returnsNewInstance () {
      State state = cortex ().state ().state ( cortex ().name ( "a" ), 1 ).state ( cortex ().name ( "b" ), 2 )
    ...
    void manyPipeCalls_allUnique () {
      ...
      for ( int i = 0; i < 100; i++ ) {
        for ( int j = i + 1; j < 100; j++ ) {
          assertNotSame ( pipes[i], pipes[j], "Pipes at index " + i + " and " + j + " should be different" );
        }
      }
    }
        ...
        for ( int i1 = 0; i1 < pipesPerThread; i1++ ) {
          for ( int t2 = t1; t2 < threadCount; t2++ ) {
            ...
            for ( int i2 = startI2; i2 < pipesPerThread; i2++ ) {
              assertNotSame ( results[t1][i1], results[t2][i2],
  ...
  class ChannelNewPipeMethods {
    ...
    void channelPipe_returnsNewInstance () {
      ...
      // Note: Channel.pipe() creates new pipe each call, but we access via
      // conduit.percept()
      // which caches. For true Channel.pipe() test, we need to get multiple pipes
      // from same
      // channel.
      ...
      Conduit < Pipe < Integer >, Integer > conduit2 = circuit.conduit ( cortex ().name ( "conduit2" ), Composer.pipe () );

      // Create pipes from same channel with different configurers - each should be
      // new
    ...
    void channelPipeWithConfigurer_returnsNewInstance () {
      ...
      Conduit < Pipe < Integer >, Integer > conduit1 = circuit.conduit ( cortex ().name ( "c1" ), composer1 );
      Conduit < Pipe < Integer >, Integer > conduit2 = circuit.conduit ( cortex ().name ( "c2" ), composer2 );
