...
final class FsState implements State {
  ...
  public Spliterator<Slot<?>> spliterator() {
    ...
    return java.util.Spliterators.spliterator(reversed,
  ...
  private State addSlot(Slot<?> slot) {
    ...
    if (last.name() == slot.name() && last.type() == slot.type() && Objects.equals(last.value(), slot.value())) {
      return this;
    }
  ...
  public <T> Stream<T> values(Slot<? extends T> slot) {
    ...
    // Collect matches into exactly-sized array in most-recent-first order (reverse
    // iteration)
