 ...
 *
 * <p>
 * Provides lazy subject creation with double-checked locking. Subject is only
 * created when {@link #subject()} is first called, avoiding allocation overhead
 * when subject is never accessed.
 *
 * <p>
...
abstract class FsSubstrate<S extends Substrate<S>> {
   ...
   *
   * @param parent
   *            the parent subject (for hierarchy)
   * @param name
  ...
  /**
   * Returns the type of this substrate. Subclasses must implement this to provide
   * their type class. This is only called once during lazy subject creation.
   *
