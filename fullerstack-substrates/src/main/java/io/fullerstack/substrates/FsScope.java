...
final class FsScope implements Scope {
  ...
  public Scope scope() {
    ...
    FsSubject<Scope> childSubject = new FsSubject<>(SCOPE_NAME, (FsSubject<?>) subject, Scope.class);
  ...
  public Scope scope(@NotNull Name name) {
    ...
    FsSubject<Scope> childSubject = new FsSubject<>(name, (FsSubject<?>) subject, Scope.class);
    FsScope child = new FsScope(childSubject, this);
    // Lazy init children list
    if (children == null) {
      children = new ArrayList<>();
